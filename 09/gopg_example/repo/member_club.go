...
func AddMemberClub() (err error) {
	...
	_, err = transaction.Model(&john).Insert()
	if !check_err(err, transaction) {
		return err
	}

	_, err = transaction.Model(&anna).Insert()
	if !check_err(err, transaction) {
		return err
	}

	_, err = transaction.Model(&bob).Insert()
	if !check_err(err, transaction) {
		return err
	}

	_, err = transaction.Model(&alice).Insert()
	if !check_err(err, transaction) {
		return err
	}
	...
	_, err = transaction.Model(&math).Insert()
	if !check_err(err, transaction) {
		return err
	}

	_, err = transaction.Model(&sport).Insert()
	if !check_err(err, transaction) {
		return err
	}

	_, err = transaction.Model(&music).Insert()
	if !check_err(err, transaction) {
		return err
	}

	//---- Thêm các thành viên vào club
	err = AssignMembersToClub(transaction, math, []model.Member{john, anna})
	if !check_err(err, transaction) {
		return err
	}

	err = AssignMembersToClub(transaction, sport, []model.Member{bob, alice})
	if !check_err(err, transaction) {
		return err
	}

	err = AssignMembersToClub(transaction, music, []model.Member{john, bob, alice})
	if !check_err(err, transaction) {
		return err
	}
...
func AssignMembersToClub(transaction *pg.Tx, club model.Club, members []model.Member) (err error) {
